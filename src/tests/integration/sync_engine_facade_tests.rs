...
async fn preview_reports_without_writing() {
    ...
    assert!(preview.resolution.conflicts.is_empty());
...
async fn invariant_rejection_escalates_through_facade() {
    ...
    // CycleOutcome::Escalated and engine.escalation_depth() without the
...
// ---------------------------------------------------------------------
// ConflictClass taxonomy — tag each UnresolvedConflict with its cause so
