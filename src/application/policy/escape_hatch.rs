...
impl MergePolicy for LastWriteWins {
    ...
    fn merge(&self, change: &FieldChange, _ctx: &MergeContext) -> MergeOutcome {
        match change.source {
            ChangeSource::A => MergeOutcome::Resolved(
                change
                    .new_from_a
                    .clone()
                    .expect("source=A implies new_from_a is Some"),
            ),
            ChangeSource::B => MergeOutcome::Resolved(
                change
                    .new_from_b
                    .clone()
                    .expect("source=B implies new_from_b is Some"),
            ),
            ChangeSource::Both => {
                use std::cmp::Ordering;
                match self.timestamp_a.cmp(&self.timestamp_b) {
                    Ordering::Greater => MergeOutcome::Resolved(
                        change.new_from_a.clone().expect("both implies Some"),
                    ),
                    Ordering::Less => MergeOutcome::Resolved(
                        change.new_from_b.clone().expect("both implies Some"),
                    ),
