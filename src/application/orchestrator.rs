...
use crate::application::policy::{
    InvariantOutcome, InvariantSet, MergeContext, PolicyMap, Resolution, UnresolvedConflict,
    resolve,
};
...
impl<A: SystemPort, B: SystemPort> Orchestrator<A, B> {
    ...
    pub fn one_way(
        ...
        let policies = crate::application::presets::one_way_from(source_name);
    ...
    pub async fn run_cycle_at(
        ...
        let resolution = resolve(&prepared.changelog, &self.policies, &ctx);
        ...
        // Tier-2: run post-merge invariants. Rejections block the cycle —
        // the Tier-1 result was structurally valid but violates a rule
        // about entity state. A Transform rewrites the value before push.
        ...
        // Push stale sides. Order is deterministic (A then B) for easier
        // reasoning in logs. A mid-sequence failure leaves the ancestor
        // untouched so the next cycle re-derives everything.
        ...
        if merged != prepared.view_a {
            ...
            self.side_a
                .upsert(
                    ...
                    canonical_id,
                    &merged,
                    prepared.fresh_ref_a.version.as_deref(),
                    &ik,
                )
                .await?;
        ...
        if merged != prepared.view_b {
            ...
            self.side_b
                .upsert(
                    ...
                    canonical_id,
                    &merged,
                    prepared.fresh_ref_b.version.as_deref(),
                    &ik,
                )
                .await?;
        ...
        // Commit the new ancestor LAST. Every earlier step can retry
    ...
    async fn prepare(
        ...
        // Bootstrap: when no ancestor exists, treat side A's current view as
        // the baseline. This makes the first cycle effectively "propagate
        // A's differences to B" and is App.md § 05 row iii's rule.
        let ancestor_key = AncestorKey::new(entity_type, canonical_id);
        let ancestor_view = match self
            .ancestor
            .get(&ancestor_key)
            .map_err(|e| SyncError::transient(e.to_string()))?
        {
