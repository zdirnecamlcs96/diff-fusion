...
//! quirks are either hidden inside the adapter or declared as
//! [`Capabilities`] so the orchestrator can plan around them.
//!
//! # Canonical-only
//!
...
/// Per-adapter feature flags. Must be declared honestly — the orchestrator
/// uses them to plan work, not to nudge the adapter into doing more than it
/// can.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct Capabilities {
    /// Adapter exposes webhook ingestion (see `parse_webhook`).
    pub supports_webhooks: bool,
    /// External system honors an ETag / version on write. If `false`, the
    /// adapter fakes OCC via read-before-write.
    pub supports_optimistic_concurrency: bool,
    /// External system accepts partial (patch-style) updates. If `false`,
    /// every upsert replaces the whole entity.
    pub supports_partial_upsert: bool,
    /// Adapter accepts a client-supplied idempotency key natively.
...
mod tests {
    ...
    fn default_capabilities_deny_everything() {
        ...
        assert!(!c.supports_optimistic_concurrency);
        assert!(!c.supports_partial_upsert);
        assert!(!c.supports_idempotency_keys);
    }
