...
└── playground/                 ← this crate (separate Cargo package)
    ...
    │   └── runs.rs             # Capture/test stores + SSE sync-progress registry
    └── web/                    # Svelte + Vite frontend (see web/README.md)
        ├── index.html          # Vite entry — loads /src/main.ts
        ├── src/                # Svelte app: App.svelte, components/, views/, lib/
        └── vite.config.ts      # Dev server proxies /api + /sync to :3000
```
...
**Policy declarations.** `pipeline.rs` parses each `per_field` entry straight
into the library's `MergePolicyRef` (from `diff_fusion::application::policy`)
and calls `.build()` — no playground-local parsing or policy types.
...
| `set_by_key` | Arrays of objects — elements matched cross-system by `identity` (single field or **composite** list); optional stable anchors re-home mutated rows | `identity` (array); optional `a_anchor` / `b_anchor` (each side's stable local PK); optional `on_both_changed` (`Escalate` (default), `PreferA`, `PreferB`, `Union`) |
...
still matches the ancestor's). Swap `on_both_changed` to `PreferA` to
...
On the items policy, cycle through `Union` → `PreferA` → `PreferB` →
`Escalate` and watch the Winner column + Merged CIF change:
`Union` keeps both IDs; `PreferA`/`PreferB` drops the other side's
...
| `Invalid policy for 'X': unknown variant `Y`, expected one of `owned_by`, `additive`, `append`, `state_machine`, `last_write_wins`, `set_by_key`` | Typo in `kind` |
| `Invalid policy for 'X': missing field `identity`` | `set_by_key` declaration is missing the `identity` field |
| `Invalid policy for 'X': unknown variant `Y`, expected one of `Escalate`, `PreferA`, `PreferB`, `Union`` | `on_both_changed` must be `Escalate`, `PreferA`, `PreferB`, or `Union` (PascalCase) |
