...
impl DiffFusion {
    ...
    pub fn from_json_str(schema_json: &str) -> Result<Self, Box<dyn Error>> {
    ...
    pub fn transform_from_str(
        &self,
        source_json: &str,
        format_id: &str,
    ) -> Result<String, Box<dyn Error>> {
        let source: Value = serde_json::from_str(source_json)?;
        let result = self.transform(&source, format_id)?;
    ...
    pub fn validate_cif(&self, value: &Value) -> Result<(), Vec<String>> {
        ...
        let cif_schema = match self.schema.get("cif_schema") {
            Some(s) => s,
            None => {
                errors.push("Schema missing 'cif_schema' definition".to_string());
                return Err(errors);
            }
        };

        if !value.is_object() {
            errors.push("CIF value must be an object".to_string());
            return Err(errors);
        }
...
/// Builder for creating DiffFusion with configuration
///
/// Use this when you need to customize behavior beyond the default.
///
/// # Examples
/// ```
/// use diff_fusion::DiffFusionBuilder;
/// use serde_json::json;
///
/// let schema = json!({"cif_schema": {}, "transformations": {}});
/// let diff_fusion = DiffFusionBuilder::new()
///     .schema(schema)
///     .build();
/// ```
pub struct DiffFusionBuilder {
    schema: Option<Value>,
}

impl DiffFusionBuilder {
    pub fn new() -> Self {
        Self { schema: None }
    }
    ...
    pub fn schema_from_str(mut self, schema_json: &str) -> Result<Self, Box<dyn Error>> {
        let schema: Value = serde_json::from_str(schema_json)?;
        self.schema = Some(schema);
        Ok(self)
    }

    pub fn build(self) -> Result<DiffFusion, Box<dyn Error>> {
        let schema = self
            .schema
            .ok_or("Schema is required to build DiffFusion")?;
        Ok(DiffFusion::new(schema))
    }
}

impl Default for DiffFusionBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_builder_pattern() {
        let result = DiffFusionBuilder::new().schema(get_test_schema()).build();
