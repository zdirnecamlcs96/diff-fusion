...
impl SyncError {
    ...
    pub fn conflict(paths: Vec<String>) -> Self {
        Self::Conflict { paths }
    }

    /// True when the cycle should retry after a delay.
    pub fn is_retriable(&self) -> bool {
        matches!(self, Self::Transient(_) | Self::StaleWrite { .. })
    }
}
...
mod tests {
    ...
    fn transient_is_retriable() {
        assert!(SyncError::transient("timeout").is_retriable());
    }

    #[test]
    fn stale_is_retriable() {
        assert!(SyncError::stale("netsuite", Some("v7".into()), "version bumped").is_retriable());
    }

    #[test]
    fn conflict_is_not_retriable() {
        assert!(!SyncError::conflict(vec!["price".into()]).is_retriable());
    }
