...
impl Changelog {
    ...
    /// Iterate over paths where both sides moved. These are the entries that
    /// the resolver must actually decide about.
    pub fn bilateral(&self) -> impl Iterator<Item = &FieldChange> {
        self.changes
            .iter()
            .filter(|c| matches!(c.source, ChangeSource::Both))
    }
}
...
mod tests {
    ...
    #[test]
    fn bilateral_filters_to_both_only() {
        let anc = json!({"x": 1, "y": 2, "z": 3});
        let a = json!({"x": 10, "y": 20, "z": 3});
        let b = json!({"x": 1, "y": 200, "z": 30});

        let log = three_way_diff(&anc, &a, &b);

        // x: A only, y: Both, z: B only
        let bilateral: Vec<_> = log.bilateral().collect();
        assert_eq!(bilateral.len(), 1);
        assert_eq!(bilateral[0].path, "y");
    }
    ...
    proptest! {
        ...
        fn swapping_a_b_inverts_source(
            ...
            prop_assert_eq!(forward.len(), swapped.len());
