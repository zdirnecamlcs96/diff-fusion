...
impl MergePolicy for StateMachine {
    ...
    fn merge(&self, change: &FieldChange, _ctx: &MergeContext) -> MergeOutcome {
        let Some(anc) = change.old_value.as_str() else {
            return non_string("ancestor");
        };
        let new_a = match side_str(change.new_from_a.as_ref(), "a") {
            Ok(v) => v,
            Err(o) => return o,
        };
        let new_b = match side_str(change.new_from_b.as_ref(), "b") {
            Ok(v) => v,
            Err(o) => return o,
        };

        let resolve_to = |to: &str, suffix: &str| -> MergeOutcome {
            if self.is_allowed(anc, to) {
                MergeOutcome::Resolved(Value::String(to.into()))
            } else {
                MergeOutcome::Conflict {
                    reason: format!("illegal transition '{anc}' -> '{to}'{suffix}"),
                }
            }
        };

        match change.source {
            ChangeSource::A => resolve_to(new_a.expect("source=A implies new_from_a is Some"), ""),
            ChangeSource::B => resolve_to(new_b.expect("source=B implies new_from_b is Some"), ""),
            ChangeSource::Both => {
                let a = new_a.expect("source=Both implies new_from_a is Some");
                let b = new_b.expect("source=Both implies new_from_b is Some");
                if a == b {
                    resolve_to(a, " (both sides)")
                } else {
                    MergeOutcome::Conflict {
                        reason: format!(
...
fn side_str<'a>(side: Option<&'a Value>, label: &str) -> Result<Option<&'a str>, MergeOutcome> {
    match side {
        Some(v) => v.as_str().map(Some).ok_or_else(|| non_string(label)),
        None => Ok(None),
    }
}
