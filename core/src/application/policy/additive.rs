...
impl MergePolicy for Additive {
    ...
    fn merge(&self, change: &FieldChange, _ctx: &MergeContext) -> MergeOutcome {
        let Some(anc) = change.old_value.as_f64() else {
            return non_numeric("ancestor");
        };
        let new_a = match side_f64(change.new_from_a.as_ref(), anc, "a") {
            Ok(n) => n,
            Err(o) => return o,
        };
        let new_b = match side_f64(change.new_from_b.as_ref(), anc, "b") {
            Ok(n) => n,
            Err(o) => return o,
        };
...
fn side_f64(side: Option<&Value>, anc: f64, label: &str) -> Result<f64, MergeOutcome> {
    match side {
        Some(v) => v.as_f64().ok_or_else(|| non_numeric(label)),
        None => Ok(anc),
    }
}
